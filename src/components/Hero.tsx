export const Hero = () => {
  return (
    <div className=' h-dvh w-full flex justify-center items-center bg-hero bg-no-repeat bg-cover bg-center '>
      <div className='absolute md:bottom-[20%] bottom-[20%] md:left-[15%]  '>
        <h1 className=' md:text-6xl  text-4xl font-bold text-white z-10 '>
          <span className='inline-block '>Soluciones</span>
          <br />
          <span className='inline-block  mt-2  text-paint-animation'>
            en pintura
          </span>
        </h1>
      </div>
    </div>
  )
}
